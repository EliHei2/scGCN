...
# initialization
data_input = 'data_input/cross_hm_2'
data_tidy  = 'data_tidy/human_mouse'
model_dir  = 'models/cross_hm_2'
out_dir    = 'output/cross_hm_2'
time_stamp = format(Sys.Date(), '%Y_%m_%d')
# dirs
## input data
adj1_f     = file.path(data_tidy, 'wi_adj_h_0.1_2020_12_12.txt')
adj2_f     = file.path(data_tidy, 'wi_adj_m_0.1_2020_12_12.txt')
...
comm_f    = file.path(data_tidy, 'gene_communities_h_0.1_2020_12_12.txt')
## labels
...
comm_out  = file.path(out_dir, sprintf('graph_comm_hc_%s_%s.pdf', '%s', time_stamp))
...
graph_col_comm(graph1, lay_nc1, graph_com$color, 5, title='hm1', labels=graph_com$GENE)

graph2  = graph_from_adjacency_matrix(adj2, 'undirected')
lay_nc2 = layout_nicely(graph2)
graph_col_comm(graph2, lay_nc2, graph_com$color, 5, title='hm2', labels=graph_com$GENE)
```
...
cell_type_graph <- function(type, title){
    ...
    grp       = colMeans(train_layers[[1]][train_idx,])
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
    # igraph::V(graph1)$color = grp
    # grp_range   = c(min(grp)^(1/2)/max(grp^(1/2)),1)
    # grp_vals    = seq(grp_range[1],grp_range[2],length.out=4)
    # grp_cols    = circlize::colorRamp2(grp_vals, viridis::viridis(4))
    # igraph::V(graph1)$color = grp_cols(grp^(1/2)/max(grp^(1/2)))
...
cell_type_graph_layer <- function(type, title){
    sprintf(graph_out, paste0(title, '_train_layer_1')) %>% pdf()
    train_idx = which(train_y$cell_type == type)
    grp       = colMeans(train_layers[[1]][train_idx,])
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
    # grp_range   = c(min(grp)^(1/2)/max(grp^(1/2)),1)
    # grp_vals    = seq(grp_range[1],grp_range[2],length.out=3)
    # grp_cols    = circlize::colorRamp2(grp_vals, viridis::viridis(3))
    # igraph::V(graph1)$color = grp_cols(grp^(1/2)/max(grp^(1/2)))
    plot.igraph(graph1,
        vertex.size = 5,
        layout = lay_nc1,
        ...
        main=title)
    dev.off()
    sprintf(graph_out, paste0(title, '_test_layer_1')) %>% pdf()
    test_idx = which(test_y$cell_type == type)
    grp       = colMeans(test_layers[[1]][test_idx,])
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
    # grp_range   = c(min(grp)^(1/2)/max(grp^(1/2)),1)
    # grp_vals    = seq(grp_range[1],grp_range[2],length.out=3)
    # grp_cols    = circlize::colorRamp2(grp_vals, viridis::viridis(3))
...
cell_type_graph_layer <- function(type, title){
    ...
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
    # grp_range   = c(min(grp)^(2)/max(grp^(2)),1)
    # grp_vals    = seq(grp_range[1],grp_range[2],length.out=3)
    # grp_cols    = circlize::colorRamp2(grp_vals, viridis::viridis(3))
    # igraph::V(graph1)$color = grp_cols(grp^(2)/max(grp^(2)))
    plot.igraph(graph1,
        vertex.size = 5,
        layout = lay_nc1,
        vertex.frame.color = igraph::V(graph1)$color,
    ...
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
    # grp_range   = c(min(grp)^(2)/max(grp^(1/2)),1)
    # grp_vals    = seq(grp_range[1],grp_range[2],length.out=3)
    # grp_cols    = circlize::colorRamp2(grp_vals, viridis::viridis(3))
    # igraph::V(graph1)$color = grp_cols(grp^(1/2)/max(grp^(1/2)))
    plot.igraph(graph1,
...
sub_idx    = sample(1:dim(raw_layer)[1], 5000)
