...
data_tidy  = 'data_tidy/human_mouse/hh'
out_dir    = 'output/human_mouse/hh'
time_stamp = format(Sys.Date(), '%Y_%m_%d')
data_input = sprintf('data_input/hh_%s', time_stamp)
...
min_counts_gene  = 1000
min_cells   = 100 

# dirs
## read
counts_mtx_f = file.path(data_raw, 'human_counts.mtx')
gene_names_f = file.path(data_raw, 'gene_names_human.tsv')
cell_meta_f  = file.path(data_raw, 'human_metadata.tsv')
## write
### analysis data
...
cell_meta    = cell_meta_f %>% fread %>% 
    ...
    setnames(c('patient_id', 'tissue', 'lib', 'barcode', 'nCounts',
        'mito_pct', ' LM22_type', 'cell_type', 'sub_type')) %>%
...
types = intersect(cell_meta_1$cell_type, cell_meta_2$cell_type)
# clean up
gc()
```
...
cell_meta_1 %<>% .[cell_type %in% types]
cell_meta_2 %<>% .[cell_type %in% types]
...
cmp_h = ifelse(cmp_h > min_cells, cmp_h, 0)
# cmp_h['CD14+ monocyte'] = 0
meta_sub_1 = cell_meta_1  %>%
    .[, .SD[sample(1:dim(.SD)[1], 
    min(..cmp_h[cell_type], ..cmp_h[cell_type]*..cmp_h[cell_type]))], by = 'cell_type']
...
hvg1 = getTopHVGs(gene_vars_1) %>% .[1:300]
...
# train & test data
adj_f     %>% fwrite(data.table(ifelse(abs(wi_h) > 0.1, 1, 0)), .)
