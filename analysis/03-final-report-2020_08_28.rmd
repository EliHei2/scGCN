...
# initialization
data_input = 'data_input/pbmc_2020_08_24'
data_tidy  = 'data_tidy/pbmc'
model_dir  = 'models/pbmc_2020_08_24'
out_dir    = 'output/pbmc_2020_08_24'
time_stamp = format(Sys.Date(), '%Y_%m_%d')
# dirs
## input data
adj1_f     = file.path(data_tidy, 'wi_adj_pbmc1_0.1_2020_08_24.txt')
adj2_f     = file.path(data_tidy, 'wi_adj_pbmc2_0.1_2020_08_24.txt')
...
comm_f    = file.path(data_tidy, 'gene_communities_pbmc1_0.1_2020_08_24.txt')
## labels
...
graph_com = comm_f %>% fread %>% 
    .[, color := brewer.pal(n=10,"Set3")[community]] %>%
    setkey(GENE) %>%
    .[colnames(adj1)]
...
sub_idx    = sample(1:dim(raw_layer)[1], 1000)
...
cell_types = c('B', 'CD14+', 'CD4+', 'Dend.', 'Mega.', 'NKT')
train_dt = data.table(type=cell_types, 
        `B`     = c(139, 2, 8, 0, 0, 1),
        `CD14+` = c(1, 133, 3, 12, 1, 0),
        `CD4+`  = c(5, 4, 136, 1, 0, 4),
        `Dend.` = c(0, 6, 2, 142, 0, 0),
        `Mega.` = c(0, 2, 0, 0, 147, 1),
        `NKT`   = c(0, 0, 3, 0, 1, 146))
conf_trian = train_dt %>% hm_conf(acc=0.9367, dat='training')
test_dt = data.table(type=cell_types, 
        `B`     = c(2366, 7, 41, 9, 21, 6),
        `CD14+` = c(2, 1207, 24, 348, 0, 1),
        `CD4+`  = c(42, 24, 2742, 13, 7, 4),
        `Dend.` = c(7, 66, 3, 125, 5, 1),
        `Mega.` = c(6, 4, 1, 1, 48, 1),
        `NKT`   = c(3, 4, 10, 4, 2, 668))
conf_test = test_dt %>% hm_conf(acc=0.9147, dat='test')
conf_hm   = conf_trian + conf_test



# Epoch 30:,                                     
# train: loss=0.003, acc=0.900                      
# val: loss=0.013, acc=0.917                        
# test: loss=0.000, acc=0.867 

# Epoch 25: acc=0.9367, loss=1.2394                 
