...
type_wagner  %<>% .$value %>% setdiff('')
type_farrell %<>% str_remove("cephalic.") %>% sort
type_wagner  %<>% 
...
## farrell
# cells_farrell = abv_farrell[which(epide), ]$barcode
cells_farrell = abv_farrell$barcode
mtx_farrell   = mtx_farrell[, c('GENE', ..cells_farrell)]
## wagner
# cells_wagner  = meta_wagner[abv_type == 'epide', ]$barcode
cells_wagner  = meta_wagner$barcode
mtx_wagner    = mtx_wagner[, c('GENE', ..cells_wagner)]
# subset matrices on shared genes
shared_genes  = intersect(mtx_wagner$GENE, mtx_farrell$GENE)
mtx_farrell   = mtx_farrell[shared_genes, ]
...
res_farrell = graph_vis(ggm_farrell, plot=T, groups=res_wagner$communities)
# res_farrell$vis_net
```
