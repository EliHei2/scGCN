...
## for later analyses
with open("data_train/cross_hm/train_idx_superCT.txt", "w") as f:
    for s in idx:
        f.write(str(s) +"\n")
...
## for later analyses
with open("data_train/cross_hm/test_idx_superCT.txt", "w") as f:
    for s in idx:
        f.write(str(s) +"\n")
...
with tf.Session() as sess:
    ...
    for layer in range(num_layers):
        ...
        embedding_df.to_csv('./models/cross_hm/' + 'train' + '_layer_{}_FC.csv'.format(layer + 1))
    # test embeddings
    n_graphs = int(test_features.shape[0])
    layer_embeddings = [np.zeros((n_graphs, 200)), np.zeros((n_graphs, 100))] 
    for i in range(n_graphs):
    ...
    for layer in range(num_layers):
        embedding_df = pd.DataFrame(data=layer_embeddings[layer].T)
        embedding_df.to_csv('./models/cross_hm/' + 'test' + '_layer_{}_FC.csv'.format(layer + 1))
