...
def load_train_classes(path):
    full_path = os.path.join(path, 'train_classes.csv')
...
def load_test_classes(path):
    full_path = os.path.join(path, 'test_classes.csv')
    classes = pd.read_csv(full_path)
    nans = pd.isna(classes['class']).values
    classes.dropna(axis=0, inplace=True)
    labels = classes['id'].values.astype(int)
...
cbmc_features, cbmc_labels, cbmc_cell_types, \
cbmc_selected_genes, _ = load_pbmc_cbmc()

np.savetxt("../../data/pbmc/proc/features.csv", pbmc_features, delimiter=",")
# np.savetxt("../../data/pbmc/proc/labels.csv", pbmc_labels, delimiter=",")
# np.savetxt("../../data/pbmc/proc/cell_types.txt", pbmc_cell_types, delimiter=",")

with open("../../data/pbmc/proc/cell_types.txt", 'w') as f:
    for item in pbmc_cell_types:
        f.write("%s\n" % item)
# np.savetxt('test.csv', pbmc_cell_types,fmt='%d',delimiter=',')

np.savetxt("../../data/cbmc/proc/features.csv", cbmc_features, delimiter=",")
# np.savetxt("../../data/cbmc/proc/labels.csv", cbmc_labels, delimiter=",")
with open("../../data/cbmc/proc/cell_types.txt", 'w') as f:
    for item in cbmc_cell_types:
        f.write("%s\n" % item)
# np.savetxt("../../data/cbmc/proc/cell_types.txt", cbmc_cell_types, delimiter=",")
# np.savetxt('test.csv', pbmc_cell_types,fmt='%d',delimiter=',')


print(pbmc_cell_types)
print(cbmc_cell_types)
