...
# read in train data
# read labels
train_labels, train_one_hot_labels, train_num_graphs, num_classes, train_nan_idx = load_classes(base_path, type='train')
...
# for later analyses
with open("data_train/pbmc/train_idx.txt", "w") as f:
    for s in idx:
        f.write(str(s) + "\n")
# sparse features
...
# read labels
test_labels, test_one_hot_labels, test_num_graphs, num_classes, test_nan_idx = load_classes(base_path, type='test')
test_class_dist = [test_labels.tolist().count(i) / test_num_graphs for i in range(num_classes)]
...
# for later analyses
with open("data_train/pbmc/test_idx.txt", "w") as f:
    for s in idx:
        f.write(str(s) + "\n")
...
if FLAGS.model == 'gcn_cheby':
    ...
    model = InceptionGCN3L(placeholders, input_dim=1, num_class=num_classes, locality_sizes=localityـsizes,
                           is_pool=True,
                           name='inception')
else:
