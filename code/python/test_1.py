...
placeholders = {
        ...
        'dropout': tf.placeholder_with_default(0.1, shape=()),
...
if FLAGS.model == 'gcn_cheby':
    ...
    model = InceptionGCN(placeholders, input_dim=1,\
...
with tf.Session() as sess:
    ...
    for epoch in range(FLAGS.epochs):
        ...
        seq = list(range(train_num_graphs))
        random.shuffle(seq)
        mini_batches = [
            seq[k:k+FLAGS.batch_size]
            for k in range(0, train_num_graphs, FLAGS.batch_size)
        ]

        for mini_batch in mini_batches:
            train_feed_dict = \
                construct_feed_dict([train_sparse_features[i] for i in mini_batch],
                                    support,
                                    [train_one_hot_labels[i] for i in mini_batch],
                                    [train_graph_weights[i] for i in mini_batch],
                                    placeholders)
            ...
            train_acc_classes[train_labels[mini_batch ], np.argmax(out, 1)[0]] += 1
            cnt += acc
            sum_loss += loss
        sum_acc = cnt / float(len(mini_batches))
        sum_sum_loss = sum_loss / float(len(mini_batches))
