...
# /Volumes/Projects/scGCN/code/R/scGCNUtils/R/save_experiment.R
...
save_experiment <- function(sce, ...){
    # TODO: message
    # set args
    params = list(...)
    if(is.list(params[[1]]))
        params = params[[1]]
    params %<>% set_params('save_experiment', 1)
    # define dirs
    meta_data     = metadata(sce)
    colData_dt    = colData(sce) %>% as.data.table
    tag           = meta_data$tag
    ## sce object
    data_tidy_dir = sprintf('data_tidy/%s', tag)
    ## train & test data
    GCN_input_dir = sprintf('data_input/%s', tag)
    adj_f         = file.path(GCN_input_dir, 'adj.txt')
    train_X_f     = file.path(GCN_input_dir, 'data_train.txt')
    train_y_f     = file.path(GCN_input_dir, 'classes_train.txt')
    test_X_f      = file.path(GCN_input_dir, 'data_test.txt')
    test_y_f      = file.path(GCN_input_dir, 'classes_test.txt')
    # save sce object
    dir.create(data_tidy_dir, showWarnings = TRUE)
    sce %>% saveRDS(data_tidy_dir)
    # save GCN input data
    ## process data
    adj = meta_data$train$graph$adj
    train_X = 
